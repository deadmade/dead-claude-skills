...
const REWRITES = [
  ...
  [re(String.raw`Use your configured architect runners \(defaults ${CURSOR_PANEL}\)\.`), `Use one runner per model: ${CLAUDE_MODELS}.`],
  [re(String.raw`Use \`arena runners\` from \`~/\.cursor/rules/pstack-models\.mdc\` when present\. Otherwise default to one each on ${CURSOR_PANEL}\.`),
    `Default to one each on ${CLAUDE_MODELS}.`],
  [re(String.raw`choose one model from the \`arena cross-judge pool\` in \`~/\.cursor/rules/pstack-models\.mdc\` when present\. Otherwise use ${CURSOR_PANEL}\. Prefer a different model family from the parent's\.`),
  ...
  [/Use the `interrogate reviewers` list from `~\/\.cursor\/rules\/pstack-models\.mdc` when present, one reviewer per entry, extending or shrinking the Reviewer A\/B\/C\/D labels below to the configured entry count\. Otherwise use the table defaults\./g,
    "Use the table below."],
  [/\| Reviewer A \| `claude-fable-5-1-thinking-max` \|/g, "| Reviewer A | `fable` |"],
  [/\| Reviewer B \| `gpt-5\.6-sol-max` \|/g, "| Reviewer B | `opus` |"],
  [/\| Reviewer C \| `grok-4\.6-fast-xhigh` \|/g, "| Reviewer C | `sonnet` |"],
  [/\| Reviewer D \| `claude-opus-5-thinking-xhigh` \|/g, "| Reviewer D | `haiku` |"],
  [/\| Subagent \| Default model \|/g, "| Subagent | Model |"],
  [/- `model`: the configured `interrogate reviewers` entry, or the table default with no configured line/g, "- `model`: the reviewer's model from the table"],
  [/If a model slug is rejected as unresolvable when you try to spawn the subagent,[^\n]*\n/g, `Valid \`model\` values for the Agent tool are ${CLAUDE_MODELS}.\n`],

  [/your configured (how-explorer|why-investigators) model \(default `grok-4\.6-fast-xhigh`\)/g, "`sonnet`"],
  [/your configured (how-explainer|why-synthesizer) model \(default `claude-fable-5-1-thinking-max`\)/g, "`fable`"],
  ...
  [/Pick the worker model from `swarm workers` in `~\/\.cursor\/rules\/pstack-models\.mdc` when present\. Otherwise use `grok-4\.6-fast-xhigh`\./g,
    "Use `sonnet` as the worker model."],
  [/Spawn all N workers in one message with `subagent_type: generalPurpose`, `environment: "cloud"`, `run_in_background: true`, and the configured model\. Use `environment: "local"` only when the worker needs access to something on the user's computer\./g,
