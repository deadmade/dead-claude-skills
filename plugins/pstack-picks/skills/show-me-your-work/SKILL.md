...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript in this workspace's transcript directory, `~/.claude/projects/<slug>/` (the newest `.jsonl`). Don't glob across `~/.claude/projects/*/`. That reads unrelated private chats. Walk the log against what actually happened:
