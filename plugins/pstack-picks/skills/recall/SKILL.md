...
Transcripts live at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the workspace path with every "/" (and other non-alphanumeric character) turned into "-" (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one chat message.
